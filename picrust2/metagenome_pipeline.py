...
def run_metagenome_pipeline(
    ...
    if "closest_reference_genome" in pred_function.columns:
        pred_function.drop("closest_reference_genome", axis=1, inplace=True)
    ...
    if "metadata_NSTI" in pred_function.columns:
        pred_function, nsti_val = drop_tips_by_nsti(
            tab=pred_function, nsti_col="metadata_NSTI", max_nsti=max_nsti
        )
    if not skip_norm:
        ...
        if "metadata_NSTI" in pred_marker.columns:
            pred_marker, nsti_val = drop_tips_by_nsti(
                tab=pred_marker, nsti_col="metadata_NSTI", max_nsti=max_nsti
            )
    ...
    if not nsti_val.empty:
        ...
        calc_weighted_nsti(
            seq_counts=study_seq_counts, nsti_input=nsti_val, outfile=weighted_nsti_out
        )
    ...
    if strat_out:
        ...
        if min_reads != 1 or min_samples != 1:
            rare_seqs = id_rare_seqs(
                in_counts=study_seq_counts, min_reads=min_reads, min_samples=min_samples
            )

    # Generate and return final tables.
    if not strat_out:
        return (None, unstrat_funcs_only_by_samples(pred_function, study_seq_counts))

    elif strat_out and not wide_table:
        return (
            metagenome_contributions(pred_function, study_seq_counts, rare_seqs),
            unstrat_funcs_only_by_samples(pred_function, study_seq_counts),
        )

    elif strat_out and wide_table:
        return strat_funcs_by_samples(pred_function, study_seq_counts, rare_seqs)


def strat_funcs_by_samples(func_abun, sample_abun, rare_seqs=[], return_unstrat=True):
    ...
    strat_func = pd.concat(
        (sample_abun.multiply(func_abun[func], axis=0) for func in func_abun.columns),
        ignore_index=True,
        axis=0,
        sort=True,
    )
    ...
    strat_func.index = pd.MultiIndex.from_product((func_abun.columns, func_abun.index))
    ...
    if len(rare_seqs) > 0:
        ...
        raw_seqs_slice.set_index("sequence", append=True, drop=True, inplace=True)
...
def norm_by_marker_copies(
    input_seq_counts, input_marker_num, norm_filename=None, round_decimal=2
):
    ...
    input_seq_counts = input_seq_counts.div(
        input_marker_num.loc[
            input_seq_counts.index.values, input_marker_num.columns.values[0]
        ],
        axis="index",
    )
    ...
    if norm_filename:
        input_seq_counts.to_csv(
            path_or_buf=norm_filename,
...
def id_rare_seqs(in_counts, min_reads, min_samples):
    ...
    few_samples_seq = set(
        in_counts[(in_counts != 0).astype(int).sum(axis=1) < min_samples].index
    )
...
def metagenome_contributions(func_abun: pd.DataFrame, sample_abun: pd.DataFrame, rare_seqs=[], skip_abun=False) -> pd.DataFrame:
    ...
    for sample in sample_abun.columns:
        ...
        func_abun_subset_melt = func_abun_subset_melt[
            func_abun_subset_melt["genome_function_count"] != 0
        ]

        if not skip_abun:

            func_abun_subset_melt["taxon_abun"] = single_abun.loc[
                func_abun_subset_melt["taxon"].to_list()
            ].to_list()

            func_abun_subset_melt["taxon_rel_abun"] = single_relabun.loc[
                func_abun_subset_melt["taxon"].to_list()
            ].to_list()

            func_abun_subset_melt["taxon_function_abun"] = (
                func_abun_subset_melt["genome_function_count"]
                * func_abun_subset_melt["taxon_abun"]
            )

            func_abun_subset_melt["taxon_rel_function_abun"] = (
                func_abun_subset_melt["genome_function_count"]
                * func_abun_subset_melt["taxon_rel_abun"]
            )

            func_abun_subset_melt["norm_taxon_function_contrib"] = (
                func_abun_subset_melt["taxon_function_abun"]
                / func_abun_subset_melt.groupby("function")
                .sum(numeric_only=True)["taxon_function_abun"][
                    func_abun_subset_melt["function"]
                ]
                .to_numpy()
            )
        ...
        rare_seqs = [
            r for r in rare_seqs if r in func_abun_subset_melt["taxon"].to_list()
        ]

        if len(rare_seqs) > 0:
            func_abun_subset_melt.loc[
                func_abun_subset_melt["taxon"].isin(rare_seqs), "taxon"
            ] = "RARE"
            func_abun_subset_melt = func_abun_subset_melt.groupby(
                ["function", "taxon"], as_index=False
            ).sum()
        ...
        if skip_abun:
            ...
            func_abun_subset_melt = func_abun_subset_melt[
                ["sample", "function", "taxon", "genome_function_count"]
            ]
        else:
            func_abun_subset_melt = func_abun_subset_melt[
                [
        ...
        if s_i == 0:
            ...
            metagenome_contrib_out = pd.concat(
                [metagenome_contrib_out, func_abun_subset_melt], axis=0
            )
...
def contrib_to_unstrat(contrib_table, sample_order=None):
    ...
    if sample_order:
        contrib_table = contrib_table.reindex(columns=sample_order, fill_value=0)
