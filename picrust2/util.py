    ...
    with open(filename, "r") as stockholm:
        ...
        for line in stockholm:
            ...
            if line_count == 0 and "# STOCKHOLM" not in line:
                sys.exit(
                    "Error - stockholm format multiple-sequence "
...
def read_seqabun(infile: str) -> pd.DataFrame:
    ...
    if mothur_format:
        input_seqabun = pd.read_csv(
            filepath_or_buffer=infile, sep="\t", dtype={"Group": str}, low_memory=False
        )
...
def three_df_index_overlap_sort(df1: pd.DataFrame, df2: pd.DataFrame, df3: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    label_overlap = df1.index.intersection(
    ...
    if len(label_overlap) == 0:
        sys.exit(
            "Stopping - no sequence ids overlap between all three of the input files."
        )

    elif len(label_overlap) < len(df1.index) * 0.5:
        print(
            "Warning: fewer than half of the sequence ids overlap between "
            "the input files.",
            file=sys.stderr,
        )
...
def check_files_exist(filepaths: List[str]) -> None:
    ...
    if num_nonexist == 0:
        ...
        sys.exit(
            "\n\nStopping - these input files were not found: "
            + ", ".join(missing_files)
        )
...
def check_empty_traits(filepaths: List[str]) -> None:
    ...
    for filepath in filepaths:
        ...
        traits_tab = pd.read_csv(
            filepath, sep="\t", low_memory=False, index_col=0, header=0
        )
    ...
    if num_nonexist == 0:
        ...
        sys.exit(
            "\n\nStopping - this input file was empty or all counts sum to 0: "
            + empty_files[0]
        )
    elif num_nonexist > 1:
        sys.exit(
            "\n\nStopping - these input files were empty or all counts sum to 0: "
            + ", ".join(empty_files)
        )
...
def add_descrip_col(inputfile: Union[str, pd.DataFrame], mapfile: str, in_df: bool = False) -> pd.DataFrame:
    ...
    if not any(map_tab.index.isin(function_tab.iloc[:, 0])):
        sys.exit(
            "Error: no function ids in input table are found "
...
def convert_humann2_to_picrust2(infiles: List[str], outfile: str, stratified: bool) -> None:
    ...
    for infile in infiles:
        first_col = str(pd.read_csv(infile, sep="\t", nrows=0).columns[0])
        humann2_single = pd.read_csv(
            infile, sep="\t", low_memory=False, dtype={first_col: str}
        )
    ...
    if len(infile_index_names_set) > 1:
        sys.exit(
            "Error input HUMAnN2 tables are not all for the same "
    ...
    if index_name == "# Pathway":
        ...
        sys.exit(
            'Error: first column of input HUMAnN2 files is "'
            + index_name
            + '". Either "# Pathway" or '
    ...
    if not stratified:
        humann2_combined.to_csv(path_or_buf=outfile, sep="\t", index_label=first_col)
    ...
    # unstratified format and split index into two columns.
        ...
        split_df = pd.DataFrame.from_records(
            list(humann2_combined.index.str.split(pat="\\|", n=1)),
        ...
        # Reorder columns.
        humann2_combined = humann2_combined.loc[
            :, [first_col, "sequence"] + original_col
        ]
...
def convert_picrust2_to_humann2(infiles: List[str], outfolder: str, stratified: bool) -> None:
    ...
    if not stratified:
        ...
        if len(infiles) > 1:
            sys.exit(
                "Stopping - only expected one input file when converting "
                "from PICRUSt2 unstratified table to HUMAnN2 format"
            )

        in_tab = pd.read_csv(
            infiles[0], sep="\t", index_col=0, low_memory=False, dtype={"function": str}
        )

        # Double-check that this table isn't stratified.
        if "sequence" in in_tab.columns:
            sys.exit(
                "Stopping - column named sequence was found in the input "
    ...
    # Otherwise read in stratified and unstratified tables.
        ...
        if len(infiles) != 2:
            ...
            sys.exit(
                "Stopping - expected two input files (stratified and "
        ...
        if strat_table_count != 1:
            sys.exit(
                "Stopping - exactly one stratified table should have been "
        ...
        if in_tab1.columns[0] != in_tab2.columns[0]:
            sys.exit(
                "Stopping - label of first column does not match between "
        ...
        def set_picrust2_tab_index(in_table):
            ...
            if "sequence" in in_table.columns:
                in_table.index = in_table.loc[:, in_table.columns[0]].str.cat(
                    others=in_table.loc[:, in_table.columns[1]], sep="|"
                )
    ...
    if index_name == "pathway":
        ...
        sys.exit(
            'Error: first column of input PICRUSt2 tables is "'
            + index_name
            + '". One of "pathway", "gene" or '
...
def convert_picrust2_to_humann2_merged(infiles: List[str], outfile: str) -> None:
    ...
    for infile in infiles:
        ...
        in_table = pd.read_csv(
            infile, sep="\t", low_memory=False, dtype={"function": str, "sequence": str}
        )
    ...
    if len(infile_index_names_set) > 1:
        sys.exit(
            "Error input PICRUSt2 tables are not all for the same "
            "datatype. The datatypes of the input files are: "
            + ", ".join(list(infile_index_names_set))
        )
...
def contrib_to_legacy(infiles: List[str], outfile: str, use_rel_abun: bool = True) -> None:
    ...
    contrib_df = pd.read_csv(
        ...
        sep="\t",
        low_memory=False,
        dtype={"function": str, "sequence": str, "sample": str, "taxon": str},
    )
    ...
    if "norm_taxon_function_contrib" in contrib_df:
        contrib_df.rename(
            columns={"norm_taxon_function_contrib": "ContributionPercentOfSample"},
            inplace=True,
        )
    ...
    if abun_col_counter == 0:
        contrib_df = contrib_df[["Gene", "Sample", "OTU", "GeneCountPerGenome"]]
    elif abun_col_counter < 4:
        sys.exit(
            "Error: certain abundance columns found in contribution "
        ...
        if use_rel_abun:
            contrib_df.rename(
                columns={
                    "taxon_rel_abun": "OTUAbundanceInSample",
                    "taxon_rel_function_abun": "CountContributedByOTU",
                },
                inplace=True,
            )
            contrib_df.drop(
                labels=["taxon_abun", "taxon_function_abun"], axis=1, inplace=True
            )
        else:
            contrib_df.rename(
                columns={
                    "taxon_abun": "OTUAbundanceInSample",
                    "taxon_function_abun": "CountContributedByOTU",
                },
                inplace=True,
            )
            contrib_df.drop(
                labels=["taxon_rel_abun", "taxon_rel_function_abun"],
                axis=1,
                inplace=True,
            )
...
def restricted_float(in_arg: str) -> float:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            in_arg + " is not a floating-point " "literal (i.e. not a proportion)"
        )
...
def shuffle_predictions(input: str, outdir: str, rep: int, seed: Optional[int] = None) -> None:
    ...
    for i in range(1, rep + 1):
        ...
        pred_table_shuffled["sequence"] = np.random.permutation(
            pred_table["sequence"].values
        )

        if input[-7:] == ".tsv.gz":
            outfile = join(
                outdir, input.replace(".tsv.gz", "_shuf" + str(i) + ".tsv.gz")
            )
        elif input[-4:] == ".tsv":
        ...
        pred_table_shuffled.to_csv(
            path_or_buf=outfile, sep="\t", index=False, compression="infer"
        )
