...
from picrust2.metagenome_pipeline import (
    strat_funcs_by_samples,
...
class PathwaysDatabase:
    ...
    def _set_pathways_structure(self, reactions, reaction_names=None):
        ...
        for pathway in reactions:
            ...
            reaction_list, key_reactions = self._find_reaction_list_and_key_reactions(
                reactions[pathway], reaction_names
            )
    ...
    def _store_pathways(self, reactions):
        ...
        for pathway in reactions:
            for reaction in reactions[pathway]:
                self.__pathways_to_reactions[pathway] = (
                    self.__pathways_to_reactions.get(pathway, []) + [reaction]
                )
                self.__reactions_to_pathways[reaction] = (
                    self.__reactions_to_pathways.get(reaction, []) + [pathway]
                )
...
def pathway_pipeline(
    ...
    logger = get_picrust_logger(verbose=verbose)
    ...
    if per_sequence_contrib:
        ...
        if not per_sequence_abun or not per_sequence_function:
            log_and_raise(
                logger,
                '"--per_sequence_contrib" option set, but at '
    ...
    if in_format == "strat" and not wide_table:
        log_and_raise(
            logger,
            "stratified table input (deprecated format), but "
    ...
    if in_format == "contrib" and wide_table and not per_sequence_contrib:
        log_and_raise(
            logger,
            'contributional table input, but "--wide_table" '
    ...
    if in_format == "contrib":
        ...
        samples = [
            col for col in in_metagenome.columns if col not in ["function", "sequence"]
        ]
    ...
    if regroup_mapfile:
        ...
        in_metagenome = regroup_func_ids(
            in_metagenome, in_format, regroup_mapfile, proc
        )
        regrouped_outfile = path.join(out_dir, "regrouped_infile.tsv")
        in_metagenome.to_csv(path_or_buf=regrouped_outfile, sep="\t", index=False)
    ...
    in_metagenome = in_metagenome[
        in_metagenome.function.isin(pathways_in.reaction_list())
    ]
    ...
    if in_format == "contrib":
        ...
        path_out_raw = Parallel(n_jobs=proc)(
            delayed(contrib_format_pathway_levels)(
                sample_id,
    ...
    # forward to process.
        ...
        path_out_raw = Parallel(n_jobs=proc)(
            delayed(unstrat_pathway_levels)(
                sample_id,
    ...
    if coverage:
        path_cov_unstrat = prep_pathway_df_out(path_raw_cov_unstrat, num_digits=10)
    ...
    if not per_sequence_contrib and in_format != "unstrat":
        ...
        if in_format == "strat":
            path_abun_strat = prep_pathway_df_out(path_raw_abun_strat, strat_index=True)
            ...
            path_abun_strat.sort_values(["sample", "function", "taxon"], inplace=True)
    ...
    if per_sequence_contrib:
        ...
        path_abun_strat, path_cov_strat, path_abun_by_seq, path_cov_by_seq = (
            per_sequence_contrib_levels(
        ...
        if wide_table:
            path_abun_unstrat_by_seq = strat_to_unstrat_counts(
                strat_df=path_abun_strat, func_col="pathway"
            )
        else:
            path_abun_unstrat_by_seq = contrib_to_unstrat(
                contrib_table=path_abun_strat,
    ...
    if len(path_abun_unstrat.index) == 0:
        log_and_raise(
            logger,
            "No pathways were identified. This "
    ...
    if strat_index:
        ...
        if not in_tab_df.empty:
            ...
            split_df = pd.DataFrame.from_records(
                list(in_tab_df.index.str.split(pat="\\|\\|\\|", n=1)),
                columns=["pathway", "sequence"],
            )
...
def read_metagenome_input(filename, **kwargs):
    ...
    if "function" not in input_df.columns:
        log_and_raise(
            logger,
            f"Required column 'function' not found in input metagenome table: {filename}",
        )
...
def path_abun_weighted_by_seq(reaction_abun, func_ids, total_sum, path_abun, pathway):
    ...
    seq_path_abun = pd.pivot_table(data=reaction_abun, index="sequence", aggfunc="sum")

    # Get weighted pathway abundance.
    strat_path_abun = (seq_path_abun / total_sum) * path_abun

    # Remove rows that are all 0.
    strat_path_abun = strat_path_abun.loc[
        strat_path_abun[strat_path_abun.columns[0]] > 0, :
    ]

    # Rename index labels to be "pathway|||sequence".
    strat_path_abun.index = [
        "|||".join([pathway, str(seq)]) for seq in strat_path_abun.index
    ]
...
def contributional_path_abun(reaction_abun, func_ids, total_sum, path_abun, pathway):
    ...
    # Get dataframe with sum of all genes per sequence.
    contrib_path = pd.pivot_table(
        data=reaction_abun,
        index=["sample", "taxon", "taxon_abun", "taxon_rel_abun"],
        aggfunc="sum",
    )
    ...
    # Get weighted pathway abundance
    contrib_path["taxon_function_abun"] = (
        contrib_path["taxon_function_abun"] / total_sum
    ) * path_abun
    contrib_path["taxon_rel_function_abun"] = (
        contrib_path["taxon_rel_function_abun"] / total_sum
    ) * path_abun

    # Remove rows that are all 0.
    contrib_path = contrib_path.loc[contrib_path["taxon_function_abun"] > 0, :]

    contrib_path["genome_function_count"] = (
        contrib_path["taxon_function_abun"] / contrib_path["taxon_abun"]
    )
...
def minpath_wrapper(
    sample_id, unstrat_input, minpath_map, minpath_outdir, print_log=False, extra_str=""
):
    ...
    minpath_in = path.join(
        minpath_outdir, str(sample_id) + extra_str + "_minpath_in.txt"
    )

    minpath_report = path.join(
        minpath_outdir, str(sample_id) + extra_str + "_minpath_report.txt"
    )

    minpath_details = path.join(
        minpath_outdir, str(sample_id) + extra_str + "_minpath_details.txt"
    )

    minpath_mps = path.join(minpath_outdir, str(sample_id) + extra_str + "_minpath.mps")
    ...
    path2minpath = path.join(
        path.dirname(path.abspath(__file__)), "MinPath", "MinPath12hmp.py"
    )
    ...
    system_call_check(
        minpath_cmd,
        print_stdout=print_log,
        print_stderr=print_log,
    )
...
def per_sequence_contrib_levels(
    sequence_abun,
    sequence_func,
    minpath_map,
    per_seq_out_dir,
    ...
    if study_seq_counts.index.name != "normalized":
        logger.debug(
            "Input abundance table is not normalized by marker gene copy "
    ...
    pred_function = pd.read_csv(sequence_func, sep="\t", dtype={"sequence": str})
    ...
    overlapping_sequence_ids = [
        col for col in pred_function.columns if col in study_seq_counts.index.values
    ]
    ...
    if regroup_map:
        pred_function = regroup_func_ids(pred_function, "unstrat", regroup_map, nproc)

    # Get pathway levels for each sequence (note that sample info is not used
    # here).
    per_seq_raw_out = Parallel(n_jobs=nproc)(
        delayed(unstrat_pathway_levels)(
            sequence,
    ...
    if wide_table:
        strat_abun = strat_funcs_by_samples(
            func_abun=path_abun_by_seq,
        ...
        strat_abun = metagenome_contributions(
            func_abun=path_abun_by_seq, sample_abun=study_seq_counts, rare_seqs=[]
        )

    if calc_coverage:

        path_cov_by_seq = prep_pathway_df_out(raw_cov, num_digits=10)
        ...
        if wide_table:
            ...
            strat_cov = strat_funcs_by_samples(
                func_abun=path_cov_by_seq,
                sample_abun=study_seq_counts,
                rare_seqs=[],
                return_unstrat=False,
            )
            ...
            strat_cov = metagenome_contributions(
                func_abun=path_cov_by_seq,
                sample_abun=study_seq_counts,
                rare_seqs=[],
                skip_abun=True,
...
def contrib_format_pathway_levels(
    sample_id,
    contrib_input,
    minpath_map,
    out_dir,
    ...
    if run_minpath:
        pathways_present = minpath_wrapper(
            sample_id, unstrat_input, minpath_map, out_dir
        )
    else:
    ...
    if calc_coverage:
        median_abun = calc_median_reaction_abun(
            reaction_abun, pathways_present, pathway_db
        )
    else:
        median_abun = None
    ...
    for pathway in pathways_present:
        ...
        path_reaction_abun = {
            reaction_id: reaction_abun[reaction_id] for reaction_id in reactions
        }
        ...
        if not per_sequence_contrib:
            ...
            strat_path_abun = contributional_path_abun(
                contrib_input,
                reactions,
            ...
            if pathway_i == 0:
                ...
                strat_abun = pd.concat([strat_abun, strat_path_abun], sort=False)

            strat_abun["sample"] = sample_id

            strat_abun = strat_abun[
                [
                    "sample",
                    "function",
                    "taxon",
...
def basic_strat_pathway_levels(
    sample_id,
    strat_input,
    minpath_map,
    out_dir,
...
def pathway_abun_and_coverage(
    pathway, pathway_db, reaction_abun, median_value, calc_coverage, gap_fill_on
):
    ...
    if pathway_db.is_structured():
        ...
        pathway_abun = compute_structured_pathway_abundance_or_coverage(
            structure, key_reactions, reaction_abun, False, median_value
        )
        if calc_coverage:
            # Calculate pathway coverage.
            pathway_cov = compute_structured_pathway_abundance_or_coverage(
                structure, key_reactions, reaction_abun, True, median_value
            )
...
def regroup_func_ids(
    in_df: pd.DataFrame, in_format: str, mapfile: str, proc: int, **kwargs
):
    """Regroup function ids in input table based on provided mapfile.

    Args:
        in_df (pd.DataFrame): Input dataframe containing function ids.
        in_format (str): Format of the input dataframe.
        mapfile (str): Path to the mapfile for regrouping function ids.

        proc (int): Number of processes to use for parallel processing.

    Returns:
        pd.DataFrame: Dataframe with regrouped function ids.
    """

    logger = kwargs.get("logger", get_picrust_logger())
    logger.debug("Regrouping function ids in input table based on provided mapfile. ")
    ...
    function_chunks = [
        functions[i : i + chunk_size] for i in range(0, len(functions), chunk_size)
    ]
    ...
    raw_new_ids_dfs = Parallel(n_jobs=proc)(
        delayed(convert_func_ids)(func_subset, in_df, func_map)
    ...
    if in_format == "contrib":
        regrouped_table = pd.pivot_table(
            raw_new_ids_combined,
            index=["sample", "function", "taxon", "taxon_abun", "taxon_rel_abun"],
            aggfunc="sum",
        )

    elif in_format == "strat":
        regrouped_table = pd.pivot_table(
            raw_new_ids_combined, index=["function", "sequence"], aggfunc="sum"
        )
    elif in_format == "unstrat":
        regrouped_table = pd.pivot_table(
            raw_new_ids_combined, index="function", aggfunc="sum"
        )
    ...
    if regrouped_table.shape[0] == 0:
        log_and_raise(
            logger,
            "No rows remain after regrouping input table. The "
...
def compute_structured_pathway_abundance_or_coverage(
    structure, key_reactions, reaction_abun, calc_coverage, median_value
):
    ...
    for item in structure[1:]:
        if isinstance(item, list):
            ...
            # pathway's abundance.
            key_reaction_scores.append(
                compute_structured_pathway_abundance_or_coverage(
                    item, key_reactions, reaction_abun, calc_coverage, median_value
                )
            )
        else:
            ...
            # parameters.
            if calc_coverage:
                reaction_score = scipy.stats.chi2.cdf(reaction_score, median_value)
    ...
    if join == ",":
        ...
        if optional_reaction_scores:
            optional_reaction_scores_filt = [
                value for value in optional_reaction_scores if value > abundance
            ]
            abundance = harmonic_mean(
                key_reaction_scores + optional_reaction_scores_filt
            )
