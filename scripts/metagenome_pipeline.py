...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="Table of predicted gene family copy numbers " "(output of hsp.py).",
)
...
parser.add_argument(
    ...
    help="Output directory for metagenome predictions. " "(default: %(default)s).",
)
...
def main():
    ...
    if args.strat_out and not args.wide_table:
        ...
        strat_pred.to_csv(
            path_or_buf=strat_outfile, sep="\t", index=False, compression="gzip"
        )

    elif args.strat_out and args.wide_table:
        strat_outfile = path.join(args.out_dir, "pred_metagenome_strat.tsv.gz")
        strat_pred.to_csv(
            path_or_buf=strat_outfile, sep="\t", index=True, compression="gzip"
        )
