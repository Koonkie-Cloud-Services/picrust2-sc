...
exp_strat_simple = path.join(
    test_dir_path, "metagenome_out", "expected_metagenome_contrib.txt.gz"
)

exp_strat_wide = path.join(
    test_dir_path, "metagenome_out", "pred_metagenome_strat.tsv.gz"
)

exp_unstrat_simple = path.join(
    test_dir_path, "metagenome_out", "pred_metagenome_unstrat_simple.tsv.gz"
)

exp_unstrat = path.join(
    test_dir_path, "metagenome_out", "pred_metagenome_unstrat.tsv.gz"
)
...
exp_strat_simple_rare = path.join(
    test_dir_path, "metagenome_out", "expected_metagenome_contrib_rare.txt.gz"
)
...
marker_predict_in = pd.read_csv(marker_predict, sep="\t", dtype={"sequence": str})
marker_predict_in.set_index("sequence", drop=True, inplace=True)

exp_strat_simple_in = pd.read_csv(
    exp_strat_simple,
    sep="\t",
    dtype={"sequence": str, "taxon": str, "function": str, "sample": str},
)

exp_strat_simple_rare_in = pd.read_csv(
    exp_strat_simple_rare,
    sep="\t",
    dtype={"sequence": str, "taxon": str, "function": str, "sample": str},
)

exp_strat_wide_in = pd.read_csv(
    exp_strat_wide, sep="\t", dtype={"sequence": str, "function": str}
)
...
exp_unstrat_simple_in = pd.read_csv(
    exp_unstrat_simple, sep="\t", dtype={"function": str}
)
...
class metagenome_pipeline_test(unittest.TestCase):
    ...
    def test_full_pipeline_strat_tsv(self):
        ...
        with TemporaryDirectory() as temp_dir:
            strat_out, unstrat_out = run_metagenome_pipeline(
                input_seqabun=seqtab_tsv_simple,
        ...
        pd.testing.assert_frame_equal(
            strat_out.reset_index(drop=True),
            exp_strat_simple_in.reset_index(drop=True),
            check_like=True,
            atol=1e-3,
        )
        pd.testing.assert_frame_equal(
            unstrat_out, exp_unstrat_simple_in, check_like=True, atol=1e-3
        )
    ...
    def test_full_pipeline_strat_wide_tsv(self):
        ...
        pd.testing.assert_frame_equal(strat_out, exp_strat_wide_in, check_like=True)
    ...
    def test_norm_by_marker_copies(self):
        ...
        test_norm = norm_by_marker_copies(
            input_seq_counts=seqtab_in,
    ...
    def test_weighted_nsti(self):
        ...
        weighted_nsti_out = calc_weighted_nsti(exp_norm_in, nsti_in, return_df=True)

        expected_weighted_nsti = {
            "samples": ["sample1", "sample2", "sample3"],
            "weighted_NSTI": [0.292857143, 0.348275862, 0.436111111],
        }
        ...
        pd.testing.assert_frame_equal(
            weighted_nsti_out, expected_weighted_nsti_df, check_like=True
        )
    ...
    def test_nsti_filtering(self):
        ...
        pred_test_in_filt, nsti_col = drop_tips_by_nsti(
            tab=pred_test_in, nsti_col="metadata_NSTI", max_nsti=0.003
        )

        expected_passing_seqs = set(
            [
                "2571042249_cluster",
                "2593339006",
                "2571042244",
        ...
        self.assertSetEqual(expected_passing_seqs, set(list(pred_test_in_filt.index)))

    def test_nsti_filtering_all_err(self):
        """Test that error thrown when all ASVs thrown out."""
        from picrust2.logger import PicrustError
        ...
        with self.assertRaises(PicrustError):
            drop_tips_by_nsti(
                tab=pred_test_in, nsti_col="metadata_NSTI", max_nsti=0.000001
            )
...
class rare_seqs_test(unittest.TestCase):
    ...
    def test_full_pipeline_strat_rare_category_tsv(self):
        ...
        pd.testing.assert_frame_equal(
            unstrat_out, exp_unstrat_simple_in, check_like=True
        )
